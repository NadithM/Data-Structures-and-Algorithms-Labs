...
public class Graph {
	...
	private void addEdge(String vertex1, String vertex2, int weight) {
		...
		String ky3;
		boolean flag=true;
		for(Map.Entry<String,Integer> entry3 : GraphApp1.nooftime.entrySet()) {
			ky3=entry3.getKey();

			if(ky3.contains(vertex1+" "+vertex2)) {
				flag=false;
				break;
			}
		...
		if(flag==true)GraphApp1.nooftime.put(vertex1+" "+vertex2+"/"+vertex2+" "+vertex1,0);
		List<neighbor> edges;
	...
	public boolean isConnected(String vertex1, String vertex2) {
		...
		for(neighbor temp: edges){
			if(temp.vertex.equals(vertex2)) return true;
